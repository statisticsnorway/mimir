...
exports.run = function(props: PublishDatasetConfig): void {
  ...
  if (dataset && dataSource.data.dataSource) {
    ...
    if (key) {
      log.info(`PublishDataset - Start publishing dataset: ${dataSource.data.dataSource?._selected} Key: ${key} for Statistikk ${statisticsId}`)
      createOrUpdateDataset(dataSource.data.dataSource?._selected, DATASET_BRANCH, key, dataset.data)
      log.info(`PublishDataset - Finish publishing dataset: ${dataSource.data.dataSource?._selected} Key: ${key} for Statistikk ${statisticsId}`)
    ...
    run(cronContext, () => {
      log.info(`PublishDataset - create task: cleanupPublishDataset_${jobId}_${statisticsId}_${dataset._name} Time: ${dateWithSleep}` )
