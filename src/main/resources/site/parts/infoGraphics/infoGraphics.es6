...
exports.get = function(req) {
  ...
  const source = i18nLib.localize({key: 'source'});
  const descriptionInfographics = i18nLib.localize({key: 'descriptionInfographics'});
  ...
  const model = { page, part, mode, selectedSource, longDesc, source, descriptionInfographics}
