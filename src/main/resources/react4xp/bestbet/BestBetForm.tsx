...
function BestBetForm(props: BestBetFormProps) {
  ...
  const selectedContentType = props.contentTypes.filter(
    (contentType) => formState.contentTypeValue === contentType.id
  )[0]
  const selectedMainSubject = props.mainSubjects.filter(
    (mainSubject) => formState.mainSubjectValue === mainSubject.title
  )[0]
  const selectedEnglishMainSubject = props.mainSubjectsEnglish.filter(
    (mainSubjectEnglish) => formState.englishMainSubjectValue === mainSubjectEnglish.title
  )[0]
    ...
    <div className='best-bet-form'>
      ...
      <Row className='d-flex flex-row align-items-end mb-3'>
        ...
        <Col className='d-flex justify-content-end'>
          <Button primary onClick={() => props.handleTag('submit')}>
            Legg til
          </Button>
        </Col>
      </Row>
        ...
        <Col>
          {formState.isXPContent && (
            <div id='content-selector-dropdown' className='ssb-dropdown mb-3'>
              <label id='dropdown-label' htmlFor='react-select-3-input'>
                Content selector
              </label>
              <AsyncSelect
          ...
          {formState.isXPContent && (
            <RadioGroup
              ...
              selectedValue={
                formState.startDateValue
                  ? formState.startDateValue === 'xp'
                    ? 'date-select-xp'
                    : 'date-select-manual'
                  : 'date-select-none'
              }
